...
/*   Updated: 2025/07/07 15:01:46 by haile         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	render(t_fractol *f)
{
	...
	while (f->pos.y <= HEIGHT)
	{
		...
		while (f->pos.x <= WIDTH)
		{
			real = f->shape.min_real + (double)f->pos.x * (f->shape.max_real
					- f->shape.min_real) / WIDTH;
			imag = f->shape.max_imag + (double)f->pos.y * (f->shape.min_imag
					- f->shape.max_imag) / HEIGHT;
			pix = pix_calculate(f, real, imag);
			if (pix == f->max_iter)
				pix_put(&f->img, f->pos.x, f->pos.y, COLOR24);
			else
				pix_put(&f->img, f->pos.x, f->pos.y, f->color[pix % COLORS]);
			f->pos.x++;
		}
		f->pos.y++;
	}
